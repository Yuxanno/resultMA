import http from 'k6/http';
import { check, sleep } from 'k6';
import { Rate } from 'k6/metrics';

// ============================================
// МЕТРИКИ
// ============================================
// Кастомная метрика для отслеживания процента ошибок
const errorRate = new Rate('errors');

// ============================================
// КОНФИГУРАЦИЯ ТЕСТА
// ============================================
export const options = {
  // Фазы нагрузки (stages)
  // Измени duration и target для настройки нагрузки
  stages: [
    { duration: '30s', target: 50 },   // Разгон: 0 → 50 пользователей за 30 сек
    { duration: '2m', target: 50 },    // Пиковая нагрузка: 50 пользователей 2 минуты
    { duration: '30s', target: 200 },  // Стресс-тест: резкий прыжок до 200 пользователей
    { duration: '20s', target: 0 },    // Охлаждение: возврат к 0 за 20 сек
  ],

  // Пороги успешности теста (thresholds)
  // Тест провален, если условия не выполнены
  thresholds: {
    'errors': ['rate<0.05'],           // Менее 5% ошибок
    'http_req_duration': ['p(95)<500'], // 95% запросов быстрее 500мс
    'http_req_failed': ['rate<0.05'],  // Менее 5% failed запросов
  },
};

// ============================================
// КОНФИГУРАЦИЯ ОКРУЖЕНИЯ
// ============================================
// Измени BASE_URL на свой адрес сервера
const BASE_URL = 'http://localhost:9999';

// ============================================
// ОСНОВНОЙ СЦЕНАРИЙ ТЕСТА
// ============================================
export default function () {
  // ----------------------------------------
  // 1. GET запрос на health check
  // ----------------------------------------
  const healthResponse = http.get(`${BASE_URL}/api/health`);
  
  // Проверки для health check
  const healthCheck = check(healthResponse, {
    'Health: статус 200': (r) => r.status === 200,
    'Health: время отклика < 500мс': (r) => r.timings.duration < 500,
  });
  
  // Записываем ошибку, если проверка провалилась
  errorRate.add(!healthCheck);

  // Пауза между запросами (1-3 секунды)
  // Имитирует реальное поведение пользователя
  sleep(Math.random() * 2 + 1);

  // ----------------------------------------
  // 2. GET запрос на /api/subjects (публичный endpoint)
  // ----------------------------------------
  const params = {
    headers: {
      'Content-Type': 'application/json',
    },
  };

  const apiResponse = http.get(`${BASE_URL}/api/subjects`, params);

  // Проверки для API endpoint
  const apiCheck = check(apiResponse, {
    'API: статус 200 или 401': (r) => r.status === 200 || r.status === 401,
    'API: время отклика < 500мс': (r) => r.timings.duration < 500,
    'API: есть тело ответа': (r) => r.body && r.body.length > 0,
  });

  // Записываем ошибку, если проверка провалилась
  errorRate.add(!apiCheck);

  // Пауза перед следующей итерацией
  sleep(1);
}

// ============================================
// ДОПОЛНИТЕЛЬНЫЕ LIFECYCLE HOOKS (опционально)
// ============================================

// Выполняется один раз перед началом теста
export function setup() {
  console.log('🚀 Запуск стресс-теста...');
  console.log(`📍 Целевой сервер: ${BASE_URL}`);
  return { startTime: new Date() };
}

// Выполняется один раз после завершения теста
export function teardown(data) {
  const endTime = new Date();
  const duration = (endTime - data.startTime) / 1000;
  console.log(`✅ Тест завершен за ${duration.toFixed(2)} секунд`);
}
