...
export default function GroupsPage() {
  ...
  const [selectedSubject, setSelectedSubject] = useState<string>(''); // Фильтр по предмету
  ...
  // Filter groups by search query and subject
  const filteredGroups = groups.filter(group => {
    ...
    const matchesSubject = !selectedSubject || 
      (group.subjectId?._id === selectedSubject || group.subjectId === selectedSubject);
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
          ...
          {subjects.map((subject) => {
            const count = groups.filter(g => 
              g.subjectId?._id === subject._id || g.subjectId === subject._id
            ).length;
            return (
              <button
                key={subject._id}
